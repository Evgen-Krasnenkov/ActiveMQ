...
public class EmployeePublisherTest {
    ...
    @Test
    public void givenEmployee_whenPublisherSends_thenEmployeeListener2Called() {
        Mockito.when(service.save(employee)).thenReturn(employee);
        employeePublisher.send(employee);
        await().atMost(TIMEOUT, TimeUnit.SECONDS).untilAsserted(
                () -> verify(employeeListener, atLeast(1)).getEmployeeListener2(any()));
        verify(employeeListener, atLeast(1)).getEmployeeListener2(any());
        verify(service, atLeast(2)).save(any());
    }

    @Test
    public void givenEmployee_whenPublisherSends_thenEmployeeListener1Called() {
        Mockito.when(service.save(employee)).thenReturn(employee);
        employeePublisher.send(employee);
        await().atMost(TIMEOUT, TimeUnit.SECONDS).untilAsserted(
                () -> verify(employeeListener, atLeast(1)).getEmployeeListener1(any()));
        verify(employeeListener, atLeast(1)).getEmployeeListener1(any());
        verify(service, atLeast(2)).save(any());
    }
